...
void AShooterAIController::FindClosestEnemy()
{
	...
	ANexosPlayerCharacter* BestPawn = NULL;

	for (ANexosPlayerCharacter* TestPawn : TActorRange<ANexosPlayerCharacter>(GetWorld()))
	{
...
bool AShooterAIController::FindClosestEnemyWithLOS(ANexosPlayerCharacter* ExcludeEnemy)
{
	...
	if (Hit.bBlockingHit == true)
	{
		...
		if (Hit.GetActor() != NULL)
		{
			...
			else if (bAnyEnemy == true)
			{
				...
				if (HitChar != NULL)
				{
					ANexosPlayerState* HitPlayerState = Cast<ANexosPlayerState>(HitChar->GetPlayerState());
					ANexosPlayerState* MyPlayerState = Cast<ANexosPlayerState>(PlayerState);
...
{
	AShooterBot* MyBot = Cast<AShooterBot>(GetPawn());
	ANexosWeaponBase* MyWeapon = MyBot ? MyBot->GetWeapon() : NULL;
	if (MyWeapon == NULL)
	{
		return;
	}
	...
	ANexosPlayerCharacter* Enemy = GetEnemy();
...
void AShooterAIController::CheckAmmo(const class ANexosWeaponBase* CurrentWeapon)
{
...
class ANexosPlayerCharacter* AShooterAIController::GetEnemy() const
{
	if (BlackboardComp)
	{
		return Cast<ANexosPlayerCharacter>(BlackboardComp->GetValue<UBlackboardKeyType_Object>(EnemyKeyID));
	}
