...
struct FDeathMessage
{
	...
	TWeakObjectPtr<class UNexosDamageType> DamageType;
...
public:
	...
	/** Sent from NexosWeapon, shows NO AMMO text. */
	 ...
	 */
	void SetMatchState(ENexosMatchState::Type NewState);

	/** Get state of current match. */
	ENexosMatchState::Type GetMatchState() const;
	 ...
	 */
	void ShowDeathMessage(class ANexosPlayerState* KillerPlayerState, class ANexosPlayerState* VictimPlayerState, const UDamageType* KillerDamageType);
...
protected:
	...
	UPROPERTY()
		FCanvasIcon PowerWeapBg;

	/** Secondary weapon background icon */
	UPROPERTY()
		FCanvasIcon AntiMatWeapBg;
	...
	/** State of match. */
	ENexosMatchState::Type MatchState;

	/** Is the scoreboard widget on screen? */
	uint32 bIsScoreBoardVisible : 1;

	/** Scoreboard widget. */
	TSharedPtr<class SNexosScoreboardWidget>	ScoreboardWidget;
